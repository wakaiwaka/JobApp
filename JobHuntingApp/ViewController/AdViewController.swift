...
class AdViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,IndicatorInfoProvider {
    
    private let indicatorInfo:IndicatorInfo = "広告・出版・マスコミ"
    private let realm = try! Realm()
    @IBOutlet weak var tableView: UITableView!
    @IBOutlet weak var addButton: UIButton!
    
    private lazy var adArray = try! Realm().objects(Company.self).filter("industoryId == 6")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let company = self.adArray[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete{
            let ad = realm.objects(Company.self).filter("id == %@",self.adArray[indexPath.row].id).first
            try! Realm().write {
                realm.delete(ad!)
